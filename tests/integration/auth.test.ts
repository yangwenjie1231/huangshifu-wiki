...
async function cleanupAuthTestData() {
  ...
  await prisma.user.deleteMany({
    where: {
      OR: [{ email: { startsWith: 'test_' } }, { email: { in: AUTH_TEST_EMAILS } }],
    },
  })
}
...
describe('Auth API - 认证接口测试', () => {
  ...
  describe('POST /api/auth/login', () => {
    ...
    it('使用正确的邮箱和密码应该成功登录', async () => {
      ...
      const cookies = Array.isArray(response.headers['set-cookie'])
        ...
        : [response.headers['set-cookie']]
      const authCookie = cookies.find((cookie: string) => cookie.startsWith('hsf_token='))
  ...
  describe('POST /api/auth/register', () => {
    ...
    it('长邮箱前缀备用昵称冲突时生成的名称不应该超过长度上限', async () => {
      ...
      expect(response.body.user.displayName).toBe(
        `${longPrefix.slice(0, AUTH_DISPLAY_NAME_MAX_LENGTH - 2)}_2`
      )
  ...
  describe('POST /api/auth/wechat/login', () => {
    ...
    it('注册关闭时应该拒绝微信新用户但允许已有微信用户登录', async () => {
      ...
      const newUserResponse = await request(app).post('/api/auth/wechat/login').send({
      ...
      const existingUserResponse = await request(app).post('/api/auth/wechat/login').send({
  ...
  describe('开放注册配置', () => {
    it('默认开启且只有超级管理员可以更新', async () => {
      ...
      const disabledRegisterResponse = await request(app).post('/api/auth/register').send({
        email: 'new_user@example.com',
        password: 'ValidPassword123!',
        displayName: 'NewUser',
      })
      ...
      const enabledRegisterResponse = await request(app).post('/api/auth/register').send({
        email: 'new_user@example.com',
        password: 'ValidPassword123!',
        displayName: 'NewUser',
      })
    ...
    it('已登录用户应该能够成功登出', async () => {
      ...
      if (response.headers['set-cookie']) {
        const cookies = Array.isArray(response.headers['set-cookie'])
          ? response.headers['set-cookie']
          : [response.headers['set-cookie']]
        const clearedCookie = cookies.find((cookie: string) => cookie.includes('hsf_token=;'))
