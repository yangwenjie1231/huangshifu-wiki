...
- 清缓存必须支持 PM2 多进程 IPC 广播，避免只清当前进程
- 静态内容不能高峰时回查 DB，必须走磁盘快照
- 注册/登录存在隐藏压力，需要验证码和登录失败锁定
- 令牌桶实现应优先采用成熟限流库，避免纯 JS 高频计算
...
3. 跨进程缓存一致性广播（PM2 IPC）
4. 多层缓存体系（内存LRU + 静态页面磁盘快照）
5. 注册/登录防刷增强（验证码 + 失败锁定）
6. 速率限制增强（全局 + 端点级别）
7. PM2多核进程守护
8. 数据库连接保护和查询优化
9. 外部API保护（Gemini/Meting等）
...
- [ ] `curl localhost:3000/api/admin/cache/clear` → 通过 IPC 清理所有 PM2 进程缓存
...
- [ ] 1. 实现功能开关系统
  ...
  **What to do**: 创建 `src/server/features/featureFlags.ts` 管理所有功能的开关状态，实现动态开关API，支持运行时切换以下功能：
  ...
  缓存清理操作必须通过 PM2 IPC 广播到所有工作进程。
  生存模式状态与阈值配置需被审计日志记录，便于回溯谁在何时触发了降级。
  ...
  **Acceptance Criteria**:
  ...
  - [ ] 手动清缓存会通过 IPC 广播到全部 PM2 进程
  - [ ] 审计日志能记录 super_admin 的手动操作和自动触发事件
...
- [ ] 2. 实现内存LRU缓存层
  ...
  **What to do**: 创建 `src/server/cache/lruCache.ts` 实现本地LRU缓存，用于缓存：
  ...
  缓存容量可配置，支持手动清除。手动清除必须通过 PM2 IPC 广播到所有进程。
  需要同时提供磁盘快照管理器：平时将高频 wiki 页面/列表页的渲染结果写入本地 `snapshots/`，静态模式直接读取该目录，不允许高峰期回查数据库后补写。
  ...
  **Acceptance Criteria**:
  ...
  - [ ] 缓存命中率统计
  - [ ] 内存使用超限时自动淘汰
  - [ ] 清缓存指令能广播到所有 PM2 worker
  - [ ] 磁盘快照可在非高峰时预热写入，并在静态模式下直接读取
...
- [ ] 3. 实现静态内容预渲染和降级响应
  ...
  **What to do**: 当 `STATIC_MODE` 开启或负载过高时：
  ...
  静态内容来源必须是磁盘快照文件（HTML/JSON），在平时访问时由低频预热任务写入，生存模式下直接 `fs.readFile` 返回，禁止高峰时回查 DB 后再渲染。
  预热任务应覆盖热门 wiki、首页列表、分类列表等最常访问内容，并以低频批处理方式刷新快照。
...
- [ ] 4. 增强速率限制
  ...
  **What to do**: 扩展现有的 `rateLimiter.ts`，实现：
  ...
  - 注册接口增加验证码/极验校验
  - 登录接口增加 IP 失败次数锁定
  - 限流实现优先使用成熟库（如 rate-limiter-flexible），避免纯 JS 高频自研令牌桶
  - 验证码与失败锁定必须在服务端强制执行，不能只靠前端校验
