...
export const AlbumCoverManager = ({ albumDocId, currentCover, onCoverUpdated }: AlbumCoverManagerProps) => {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await apiPost(`/api/albums/${albumDocId}/covers`, { assetId: result.assetId });
      show('封面上传成功');
      fetchCovers();
    } catch (error) {
    ...
    <div className="fixed inset-0 z-[120] bg-black/40 p-4 flex items-center justify-center">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-hidden bg-white rounded border border-[#e0dcd3] flex flex-col">
        <header className="px-5 py-4 border-b border-[#e0dcd3] flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold text-[#2c2c2c]">专辑封面管理</h3>
            <p className="text-xs text-[#9e968e] mt-0.5">上传、管理专辑封面或将封面同步到歌曲</p>
          </div>
          <button
