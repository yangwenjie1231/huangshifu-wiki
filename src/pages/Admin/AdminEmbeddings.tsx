...
const AdminEmbeddings = () => {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-text-primary tracking-[0.12em]">向量 Embeddings 管理</h2>
        <button onClick={fetchStatus} className="px-4 py-2 border border-border text-text-secondary hover:text-brand-gold hover:border-brand-gold rounded text-sm transition-all inline-flex items-center gap-1.5">
        ...
        <>
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex items-center gap-2 ml-auto">
                  ...
                  {status.textModelLoaded ? (
                    <span className="px-2 py-0.5 rounded text-[10px] font-medium theme-status-success">已加载</span>
                  ) : status.modelErrors.text ? (
                    <span className="px-2 py-0.5 rounded text-[10px] font-medium theme-status-error">加载失败</span>
                  ) : (
                    <span className="px-2 py-0.5 rounded text-[10px] font-medium bg-bg-tertiary text-text-secondary">未加载</span>
                  )}
          ...
          <div className="bg-surface border border-border rounded p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div><span className="text-text-muted">模型：</span><span className="font-medium text-text-primary">{status.modelName}</span></div>
              <div><span className="text-text-muted">向量维度：</span><span className="font-medium text-text-primary">{status.vectorSize}</span></div>
              <div><span className="text-text-muted">集合名称：</span><span className="font-medium text-text-primary">{status.qdrantCollection}</span></div>
            </div>
            <div className="mt-4 pt-4 border-t border-border">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] text-text-muted">({status.usingModelScope ? 'ModelScope 镜像' : 'Hugging Face'})</span>
              </div>
      ...
      {showErrors && (
        <div className="bg-surface border border-border rounded p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowErrors(false)} className="p-1 rounded text-text-muted hover:text-text-primary hover:bg-surface-alt transition-colors">
              <XCircle size={18} />
            </button>
          </div>
          {loadingErrors ? (
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={clsx('px-1.5 py-0.5 rounded text-[10px] font-medium',
                        error.sourceType === 'gallery' ? 'bg-surface-alt text-brand-gold' :
                        error.sourceType === 'wiki' ? 'bg-brand-gold/15 text-brand-gold' :
