...
import { AlertTriangle, BarChart3, FileText, Image, Loader2, RefreshCw, Trash2, XCircle } from 'lucide-react'
...
const SkeletonBlock = ({ className }: { className?: string }) => (
  <div className={clsx('animate-pulse bg-[#f0ece3] rounded', className)} />
)
...
const LoadingSkeleton = () => (
  <div className="space-y-5">
    ...
    {[1, 2, 3].map((i) => (
      <div key={i} className="bg-white border border-[#e0dcd3] rounded p-5">
        <SkeletonBlock className="h-4 w-24 mb-3" />
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[1, 2, 3, 4].map((j) => (
            <div key={j} className="border border-[#e0dcd3] rounded p-4">
...
const getSourceLabel = (error: EmbeddingsError) => {
  switch (error.sourceType) {
    case 'gallery': return error.gallery?.title || `图库 #${error.galleryImageId}`
    case 'wiki': return `百科: ${error.wikiPageSlug || error.sourceId}`
    case 'post': return `帖子: ${error.postId || error.sourceId}`
    default: return error.sourceId || '未知'
  }
}
...
const AdminEmbeddings = () => {
  ...
  const [textSourceType, setTextSourceType] = useState<'all' | 'wiki' | 'post' | 'music' | 'album'>('all')
  const { show } = useToast()
  ...
  const openConfirm = (variant: 'danger' | 'warning' | 'info', title: string, message: string, onConfirm: () => void) => {
  ...
  useEffect(() => { if (showErrors) fetchErrors() }, [showErrors, selectedType])

  useEffect(() => {
    if (!status) return
    const summary = normalizeSummary(status.summary)
    let hasProcessing = summary.gallery.processing > 0 || summary.wiki.processing > 0 || summary.post.processing > 0
    if (status.textSummary) {
      const ts = status.textSummary
      if (ts.wiki.processing > 0 || ts.post.processing > 0 || ts.music.processing > 0 || ts.album.processing > 0) {
        hasProcessing = true
      }
    }
  ...
  const handleEnqueueMissing = async () => {
    ...
    try {
      const response = await apiPost<{ gallery?: { requested: number; queued: number }; wiki?: { requested: number; queued: number }; post?: { requested: number; queued: number } }>('/api/embeddings/enqueue-missing', { limit: enqueueLimit, type: selectedType })
      const parts: string[] = []
  ...
  const handleSyncBatch = async () => {
    ...
    try {
      const response = await apiPost<{ gallery?: { ready: number; failed: number }; wiki?: { ready: number; failed: number }; post?: { ready: number; failed: number } }>('/api/embeddings/sync-batch', { limit: 100, type: selectedType })
      const parts: string[] = []
  ...
  const handleRetryFailed = async () => {
    ...
    try {
      const response = await apiPost<{ gallery?: { resetCount: number }; wiki?: { resetCount: number }; post?: { resetCount: number } }>('/api/embeddings/retry-failed', { type: selectedType })
  ...
  const handleRebuildAll = async () => {
    ...
    try {
      const response = await apiPost<{ gallery?: { resetCount: number }; wiki?: { resetCount: number }; post?: { resetCount: number } }>('/api/embeddings/rebuild-all', { type: selectedType })
      const totalReset = (response.gallery?.resetCount ?? 0) + (response.wiki?.resetCount ?? 0) + (response.post?.resetCount ?? 0)
  ...
  const handleTextEnqueue = async () => {
    ...
    try {
      const response = await apiPost<{ wiki?: { requested: number; queued: number }; post?: { requested: number; queued: number }; music?: { requested: number; queued: number }; album?: { requested: number; queued: number } }>('/api/embeddings/text/enqueue', { sourceType: textSourceType, limit: textEnqueueLimit })
      const parts: string[] = []
      if (response.wiki) parts.push(`百科 ${response.wiki.queued} 个`)
      if (response.post) parts.push(`帖子 ${response.post.queued} 个`)
    ...
    finally { setTextActionLoading(null) }
  }

  const handleTextSync = async () => {
    setTextActionLoading('sync')
    try {
      const response = await apiPost<{ wiki?: { ready: number; failed: number }; post?: { ready: number; failed: number }; music?: { ready: number; failed: number }; album?: { ready: number; failed: number } }>('/api/embeddings/text/sync', { limit: 100 })
      const parts: string[] = []
      if (response.wiki) parts.push(`百科 ${response.wiki.ready}/${response.wiki.ready + response.wiki.failed}`)
      if (response.post) parts.push(`帖子 ${response.post.ready}/${response.post.ready + response.post.failed}`)
      if (response.music) parts.push(`音乐 ${response.music.ready}/${response.music.ready + response.music.failed}`)
      if (response.album) parts.push(`专辑 ${response.album.ready}/${response.album.ready + response.album.failed}`)
      show(parts.length > 0 ? `批量同步完成: ${parts.join(', ')}` : '批量同步已启动')
      fetchStatus()
    } catch { show('批量同步失败', { variant: 'error' }) }
    finally { setTextActionLoading(null) }
  ...
  const handleTextRetryFailed = async () => {
    ...
    try {
      const response = await apiPost<{ wiki?: { resetCount: number }; post?: { resetCount: number }; music?: { resetCount: number }; album?: { resetCount: number } }>('/api/embeddings/text/retry-failed', { sourceType: textSourceType })
      const totalReset = (response.wiki?.resetCount ?? 0) + (response.post?.resetCount ?? 0) + (response.music?.resetCount ?? 0) + (response.album?.resetCount ?? 0)
      show(`已重置 ${totalReset} 个失败任务并开始重新同步`)
      fetchStatus()
    } catch { show('重试失败', { variant: 'error' }) }
    finally { setTextActionLoading(null) }
  ...
  const handleTextRebuildAll = async () => {
    ...
    try {
      const response = await apiPost<{ wiki?: { resetCount: number }; post?: { resetCount: number }; music?: { resetCount: number }; album?: { resetCount: number } }>('/api/embeddings/text/rebuild-all', { sourceType: textSourceType })
      const totalReset = (response.wiki?.resetCount ?? 0) + (response.post?.resetCount ?? 0) + (response.music?.resetCount ?? 0) + (response.album?.resetCount ?? 0)
      show(`已重置 ${totalReset} 条记录并开始重建`)
      fetchStatus()
    } catch { show('重建失败', { variant: 'error' }) }
  ...
  const imageFailedTotal = summary ? summary.gallery.failed + summary.wiki.failed + summary.post.failed : 0
      ...
      {status && summary && (
        <>
          ...
          {status.textSummary && (
            <div className="bg-white border border-[#e0dcd3] rounded p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-sm font-semibold text-[#6b6560]">文本向量</h3>
                <div className="flex items-center gap-2 ml-auto">
                  <span className="text-xs text-[#9e968e]">文本模型</span>
                  ...
                  <span className="text-xs text-[#9e968e]">分词器</span>
              ...
              <div className="space-y-3">
                {([
                  ...
                  <div key={section.key}>
                    <p className="text-xs text-[#9e968e] mb-2">{section.title}</p>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <StatCard label="等待中" value={status.textSummary[section.key].pending} />
                      <StatCard label="处理中" value={status.textSummary[section.key].processing} accent="text-[#c8951e]" />
          ...
          <div className="bg-white border border-[#e0dcd3] rounded p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-[#6b6560]">图片向量批量操作</h3>
                  ...
                  <button
                    ...
                    className={clsx(
                      ...
                      selectedType === opt.value
                        ...
                        : 'bg-[#f7f5f0] text-[#6b6560] hover:text-[#c8951e]'
                    )}
                  >
                    {opt.label}
                  </button>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button onClick={() => openConfirm('warning', '批量同步确认', '确定要批量同步向量吗？这可能需要一些时间。', handleSyncBatch)} disabled={actionLoading !== null} className="px-4 py-2 rounded border border-[#e0dcd3] text-[#6b6560] text-sm font-medium hover:text-[#c8951e] hover:border-[#c8951e] disabled:opacity-50 inline-flex items-center gap-2 transition-all">
              ...
              <button onClick={() => openConfirm('warning', '重试确认', '确定要重试所有失败的向量任务吗？', handleRetryFailed)} disabled={actionLoading !== null || imageFailedTotal === 0} className="px-4 py-2 rounded border border-[#e0dcd3] text-[#6b6560] text-sm font-medium hover:text-[#c8951e] hover:border-[#c8951e] disabled:opacity-50 inline-flex items-center gap-2 transition-all">
                {actionLoading === 'retry' ? <Loader2 size={14} className="animate-spin" /> : <RefreshCw size={14} />} 重试失败
              </button>
              <button onClick={() => openConfirm('danger', '重建全部确认', '确定要重建所有向量吗？这将删除现有向量并重新生成，耗时较长。此操作不可逆。', handleRebuildAll)} disabled={actionLoading !== null} className="px-4 py-2 rounded border border-red-200 bg-red-50 text-red-600 text-sm font-medium hover:bg-red-100 disabled:opacity-50 inline-flex items-center gap-2 transition-all">
          ...
          <div className="bg-white border border-[#e0dcd3] rounded p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-[#6b6560]">文本向量批量操作</h3>
              <div className="flex gap-1">
                {[
                  { value: 'all', label: '全部' },
                  { value: 'wiki', label: '百科' },
            ...
            <div className="flex flex-wrap gap-3">
              <div className="flex items-center gap-2">
                <input
                  ...
                  min={1} max={2000}
                />
                <button onClick={handleTextEnqueue} disabled={textActionLoading !== null} className="px-4 py-2 rounded bg-[#c8951e] text-white text-sm font-medium hover:bg-[#dca828] disabled:opacity-50 inline-flex items-center gap-2 transition-all">
                  {textActionLoading === 'enqueue' ? <Loader2 size={14} className="animate-spin" /> : null} 补齐缺失
                </button>
              </div>
              <button onClick={() => openConfirm('warning', '批量同步确认', '确定要批量同步文本向量吗？这可能需要一些时间。', handleTextSync)} disabled={textActionLoading !== null} className="px-4 py-2 rounded border border-[#e0dcd3] text-[#6b6560] text-sm font-medium hover:text-[#c8951e] hover:border-[#c8951e] disabled:opacity-50 inline-flex items-center gap-2 transition-all">
                {textActionLoading === 'sync' ? <Loader2 size={14} className="animate-spin" /> : null} 批量同步
              </button>
              <button onClick={() => openConfirm('warning', '重试确认', '确定要重试所有失败的文本向量任务吗？', handleTextRetryFailed)} disabled={textActionLoading !== null || textFailedTotal === 0} className="px-4 py-2 rounded border border-[#e0dcd3] text-[#6b6560] text-sm font-medium hover:text-[#c8951e] hover:border-[#c8951e] disabled:opacity-50 inline-flex items-center gap-2 transition-all">
                {textActionLoading === 'retry' ? <Loader2 size={14} className="animate-spin" /> : <RefreshCw size={14} />} 重试失败
              </button>
              <button onClick={() => openConfirm('danger', '重建全部确认', '确定要重建所有文本向量吗？这将删除现有向量并重新生成，耗时较长。此操作不可逆。', handleTextRebuildAll)} disabled={textActionLoading !== null} className="px-4 py-2 rounded border border-red-200 bg-red-50 text-red-600 text-sm font-medium hover:bg-red-100 disabled:opacity-50 inline-flex items-center gap-2 transition-all">
      ...
      {showErrors && (
        <div className="bg-white border border-[#e0dcd3] rounded p-5">
          ...
          {loadingErrors ? (
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={clsx('px-1.5 py-0.5 rounded text-[10px] font-medium',
                        error.sourceType === 'gallery' ? 'bg-blue-50 text-blue-600' :
                        error.sourceType === 'wiki' ? 'bg-purple-50 text-purple-600' :
                        'bg-orange-50 text-orange-600'
                      )}>
                        {error.sourceType === 'gallery' ? '图库' : error.sourceType === 'wiki' ? '百科' : '帖子'}
                      </span>
                      <StatusBadge status={error.status} />
                      <span className="text-xs text-[#9e968e]">{format(new Date(error.updatedAt), 'yyyy-MM-dd HH:mm')}</span>
                    </div>
                    <p className="text-sm text-[#2c2c2c] truncate">{getSourceLabel(error)}</p>
            ...
            <div className="text-center py-8 text-[#9e968e]">
