...
export const AdminLocks = () => {
  ...
  const toggleAll = () => {
    setSelectedIds((prev) => (prev.size === data.length ? new Set() : new Set(data.map((item) => item.id))));
  };
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Trash2 size={14} /> {batchReleasing ? '释放中...' : `释放选中 ${selectedIds.size || ''}`}
          </button>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-5 py-16 text-center text-text-muted italic">暂无编辑锁</td></tr>
              )}
            </tbody>
          </table>
        </div>
