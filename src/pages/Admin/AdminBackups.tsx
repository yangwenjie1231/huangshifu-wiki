...
import { AlertTriangle, Database, Download, Loader2, Pencil, RefreshCw, Trash2, Upload, XCircle } from 'lucide-react';
...
const AdminBackups = () => {
  ...
  const handleUpdateNote = async () => {
    ...
    try {
      ...
      setBackups((current) => current.map((item) => (
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setDialog('create')} className="px-4 py-2 bg-brand-gold-dark text-white rounded text-sm font-medium hover:bg-brand-gold transition-all inline-flex items-center gap-1.5">
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-surface-alt border-b border-border">
              <th className="px-5 py-3 text-left text-[11px] font-semibold text-text-muted uppercase tracking-wider">文件名</th>
              <th className="px-5 py-3 text-left text-[11px] font-semibold text-text-muted uppercase tracking-wider">备注</th>
              <th className="px-5 py-3 text-left text-[11px] font-semibold text-text-muted uppercase tracking-wider">创建时间</th>
              <th className="px-5 py-3 text-left text-[11px] font-semibold text-text-muted uppercase tracking-wider">大小</th>
              <th className="px-5 py-3 text-right text-[11px] font-semibold text-text-muted uppercase tracking-wider">操作</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {backups.length > 0 ? (
              backups.map((backup) => (
                <tr key={backup.filename} className="hover:bg-surface-alt transition-colors">
                  ...
                  <td className="px-5 py-4 text-sm text-text-secondary max-w-[280px]">
                    {backup.note ? (
                      <span className="line-clamp-2 whitespace-pre-wrap break-words">{backup.note}</span>
                    ) : (
                      <span className="text-text-muted italic">无备注</span>
                    )}
                  </td>
                  <td className="px-5 py-4 text-sm text-text-secondary">{format(new Date(backup.createdAt), 'yyyy-MM-dd HH:mm:ss')}</td>
                  <td className="px-5 py-4 text-sm text-text-secondary">{backup.sizeFormatted}</td>
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openNoteDialog(backup)} className="p-2 text-text-secondary hover:text-brand-gold hover:bg-surface-alt rounded transition-all" title="编辑备注">
                        <Pencil size={18} />
                      </button>
              ...
              <tr>
                <td colSpan={5} className="px-5 py-16 text-center text-text-muted italic">暂无备份记录</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        >
          <div className="floating-panel bg-surface border border-border rounded w-full max-w-md mx-4 p-6 space-y-5" onClick={(e) => e.stopPropagation()}>
            ...
            {visibleDialog === 'create' && (
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-2">备份备注（可选）</label>
                <textarea
                  aria-label="备份备注（可选）"
                  value={createNote}
                ...
                />
                <p className="mt-1.5 text-xs text-text-muted text-right">{createNote.length}/{CONTENT_LIMITS.admin.backupNote}</p>
              </div>
            )}

            {visibleDialog === 'note' && (
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-2">备份备注</label>
                <textarea
                  aria-label="备份备注"
                  value={editNote}
                  ...
                  className="w-full px-4 py-2.5 rounded border border-border text-sm focus:outline-none focus:border-brand-gold resize-y"
                />
                <p className="mt-1.5 text-xs text-text-muted text-right">{editNote.length}/{CONTENT_LIMITS.admin.backupNote}</p>
              </div>
            )}
            ...
            {visibleDialog === 'restore' && (
              <div>
                ...
                />
                {restoreFile && (
                  <p className="mt-1.5 text-xs text-text-muted">已选择: {restoreFile.name} ({(restoreFile.size / (1024 * 1024)).toFixed(1)} MB)</p>
                )}
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                onClick={() => { if (visibleDialog === 'create') handleCreate(); else if (visibleDialog === 'restore') handleRestore(); else if (visibleDialog === 'delete') handleDelete(); else if (visibleDialog === 'note') handleUpdateNote(); }}
