...
export default function AdminMarkdownLinks() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-2xl font-bold text-text-primary tracking-[0.12em]">Markdown 链接批量更新</h2>
        <p className="text-sm text-text-muted mt-1">扫描和批量更新 Wiki 页面中的资源链接，支持存储策略切换。</p>
      </div>

      {/* 链接分布扫描 */}
      <div className="bg-surface border border-border rounded p-5">
        <h3 className="text-sm font-semibold text-text-primary mb-4">链接分布扫描</h3>
        <button onClick={handleScan} disabled={scanning} className="px-4 py-2 bg-brand-gold-dark text-white rounded text-sm font-medium hover:bg-brand-gold disabled:opacity-50 transition-all">
          {scanning ? '扫描中...' : '开始扫描'}
        </button>
        {scanResult && (
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-surface border border-border rounded p-4"><div className="text-2xl font-bold text-text-primary">{scanResult.totalPages}</div><div className="text-xs text-text-muted">Wiki 页面</div></div>
            <div className="bg-surface border border-border rounded p-4"><div className="text-2xl font-bold text-brand-gold">{scanResult.localLinkCount}</div><div className="text-xs text-text-muted">本地链接</div></div>
            <div className="bg-surface border border-border rounded p-4"><div className="text-2xl font-bold text-brand-gold">{scanResult.s3LinkCount}</div><div className="text-xs text-text-muted">S3 链接</div></div>
            <div className="bg-surface border border-border rounded p-4"><div className="text-2xl font-bold text-brand-gold">{scanResult.externalLinkCount}</div><div className="text-xs text-text-muted">外部链接</div></div>
          </div>
        )}
      </div>
      ...
      <div className="bg-surface border border-border rounded p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <select value={fromStorage} onChange={(e) => setFromStorage(e.target.value as any)} className="w-full px-3 py-2 border border-border rounded text-sm focus:outline-none focus:border-brand-gold">
              <option value="local">本地存储</option><option value="s3">S3 存储</option><option value="external">外部图床</option>
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1">目标存储</label>
            <select value={toStorage} onChange={(e) => setToStorage(e.target.value as any)} className="w-full px-3 py-2 border border-border rounded text-sm focus:outline-none focus:border-brand-gold">
              <option value="local">本地存储</option><option value="s3">S3 存储</option><option value="external">外部图床</option>
            </select>
          </div>
          <div className="flex items-end">
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={dryRun} onChange={(e) => setDryRun(e.target.checked)} className="w-4 h-4 accent-brand-gold" />
              <span className="text-sm text-text-secondary">预览模式（不实际修改）</span>
            </label>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div><label className="block text-sm font-medium text-text-secondary mb-1">本地基础 URL</label><input type="text" value={storageConfig.localBaseUrl} onChange={(e) => setStorageConfig({ ...storageConfig, localBaseUrl: e.target.value })} className="w-full px-3 py-2 border border-border rounded text-sm focus:outline-none focus:border-brand-gold" placeholder="/uploads/" /></div>
          <div><label className="block text-sm font-medium text-text-secondary mb-1">S3 基础 URL</label><input type="text" value={storageConfig.s3BaseUrl} onChange={(e) => setStorageConfig({ ...storageConfig, s3BaseUrl: e.target.value })} className="w-full px-3 py-2 border border-border rounded text-sm focus:outline-none focus:border-brand-gold" placeholder="https://s3.example.com/" /></div>
          <div><label className="block text-sm font-medium text-text-secondary mb-1">外部图床基础 URL</label><input type="text" value={storageConfig.externalBaseUrl} onChange={(e) => setStorageConfig({ ...storageConfig, externalBaseUrl: e.target.value })} className="w-full px-3 py-2 border border-border rounded text-sm focus:outline-none focus:border-brand-gold" placeholder="https://external.com/" /></div>
        </div>
        <div className="flex gap-2">
          <button onClick={handleSwitchStorage} disabled={updating || fromStorage === toStorage} className="px-4 py-2 bg-brand-gold-dark text-white rounded text-sm font-medium hover:bg-brand-gold disabled:opacity-50 transition-all">
            {updating ? '处理中...' : dryRun ? '预览切换' : '执行切换'}
          </button>
          <button onClick={handleSyncWithImageMap} disabled={updating} className="px-4 py-2 border border-border text-text-secondary hover:text-brand-gold hover:border-brand-gold rounded text-sm font-medium disabled:opacity-50 transition-all">
            同步 ImageMap
          </button>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded p-5">
        ...
        {mappings.map((mapping, index) => (
          <div key={index} className="flex gap-2 mb-2 items-start">
            <input type="text" value={mapping.oldUrl} onChange={(e) => updateMapping(index, 'oldUrl', e.target.value)} className="flex-1 px-3 py-2 border border-border rounded text-sm focus:outline-none focus:border-brand-gold" placeholder="原始链接" />
            <input type="text" value={mapping.newUrl} onChange={(e) => updateMapping(index, 'newUrl', e.target.value)} className="flex-1 px-3 py-2 border border-border rounded text-sm focus:outline-none focus:border-brand-gold" placeholder="新链接" />
        ...
        <div className="flex gap-2 mt-4">
          <button onClick={handlePreview} disabled={previewing} className="px-4 py-2 border border-border text-text-secondary hover:text-brand-gold hover:border-brand-gold rounded text-sm font-medium disabled:opacity-50 transition-all">
            {previewing ? '预览中...' : '预览效果'}
          </button>
          <button onClick={handleUpdate} disabled={updating} className="px-4 py-2 bg-brand-gold-dark text-white rounded text-sm font-medium hover:bg-brand-gold disabled:opacity-50 transition-all">
            ...
            {previewResult.map((item) => (
              <div key={item.slug} className="p-3 bg-surface-alt rounded">
                ...
                {item.preview.replaced ? (
                  ...
                  <div className="mt-2 text-xs text-text-muted">无变化</div>
                )}
              </div>
            ))}
          </div>
