...
const Home = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      <section className="relative h-[60vh] min-h-[400px] md:h-[70vh] md:min-h-[500px] rounded-2xl md:rounded-3xl overflow-hidden mb-12 md:mb-20 shadow-lg md:shadow-2xl">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-6 md:p-12 lg:p-20">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <h1 className="text-white text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-serif font-bold mb-4 md:mb-6 leading-tight">
              黄诗扶 <span className="text-2xl sm:text-3xl md:text-4xl font-normal italic opacity-80 block sm:inline ml-0 sm:ml-4">Huang Shifu</span>
            </h1>
            <p className="text-white/80 text-base md:text-xl font-serif italic max-w-2xl mb-6 md:mb-10 leading-relaxed">
              "以诗入乐，以乐咏诗。在这里，探索关于黄诗扶的一切。"
            </p>
            <div className="flex flex-wrap gap-3 md:gap-4">
              <Link to="/wiki" className="px-6 md:px-8 py-3 md:py-4 bg-brand-primary text-gray-900 rounded-full font-bold hover:scale-105 transition-all flex items-center gap-2 shadow-lg touch-target-lg">
                进入百科 <ArrowRight size={18} />
              </Link>
              <Link to="/forum" className="px-6 md:px-8 py-3 md:py-4 bg-white/10 backdrop-blur-md border border-white/30 text-white rounded-full font-medium hover:bg-white/20 transition-all">
                参与社区讨论
              </Link>
            </div>
          </motion.div>
      ...
      <section className="mb-12 md:mb-24">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-8 md:mb-12">
          <div>
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-gray-900 mb-2">百科全书</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
          {[
            { title: '人物介绍', icon: <Book size={28} />, desc: '生平经历、艺术风格与成就', color: 'bg-white', link: '/wiki?category=biography' },
            { title: '音乐作品', icon: <Music size={28} />, desc: '原创、翻唱及合作曲目全收录', color: 'bg-white', link: '/music' },
            { title: '专辑一览', icon: <Book size={28} />, desc: '历年发行专辑与EP详情', color: 'bg-white', link: '/wiki?category=album' },
            { title: '活动记录', icon: <Calendar size={28} />, desc: '演出、直播与线下活动时间线', color: 'bg-white', link: '/wiki?category=event' },
          ].map((cat, i) => (
            <motion.div key={cat.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }} className={`${cat.color} p-5 md:p-8 rounded-xl md:rounded-2xl hover:shadow-lg transition-all group cursor-pointer border border-gray-100 hover:border-brand-primary/20`}>
              <div className="text-brand-primary mb-4 md:mb-6 group-hover:scale-110 transition-transform">{cat.icon}</div>
              <h3 className="text-xl md:text-2xl font-serif font-bold mb-2 md:mb-3">{cat.title}</h3>
              <p className="text-gray-500 text-sm leading-relaxed mb-4 md:mb-6">{cat.desc}</p>
              <Link to={cat.link} className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-brand-cream border border-gray-100 flex items-center justify-center text-brand-primary group-hover:bg-brand-primary group-hover:text-gray-900 transition-all">
                <ArrowRight size={16} />
              </Link>
            </motion.div>
          ))}
      ...
      {feed?.hotPosts && feed.hotPosts.length > 0 && (
        <section className="mb-12 md:mb-24">
          <div className="flex justify-between items-end mb-6 md:mb-8">
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-gray-900 flex items-center gap-2">🔥 热门帖子</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            {feed.hotPosts.slice(0, 3).map((post) => (
              <Link key={post.id} to={`/forum/${post.id}`} className="block bg-gradient-to-br from-brand-primary/5 to-brand-primary/10 p-5 md:p-6 rounded-xl md:rounded-2xl border border-brand-primary/10 hover:border-brand-primary/30 transition-all cursor-pointer group">
                ...
                <h4 className="text-base md:text-lg font-serif font-bold mb-2 group-hover:text-brand-primary transition-colors line-clamp-2">{post.title}</h4>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
        <div className="lg:col-span-2">
          <div className="flex justify-between items-end mb-6 md:mb-8">
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-gray-900">社区动态</h2>
            <Link to="/forum" className="text-brand-primary font-bold text-sm hover:underline">更多讨论</Link>
          </div>
          <div className="space-y-3 md:space-y-4">
            {loading ? (
              [1, 2, 3].map(i => <div key={i} className="h-20 md:h-24 bg-white rounded-xl md:rounded-2xl animate-pulse border border-gray-100"></div>)
            ) : (feed?.recentPosts?.length ?? 0) > 0 ? (
              feed!.recentPosts.slice(0, 5).map((post) => (
                <Link key={post.id} to={`/forum/${post.id}`} className="block bg-white p-4 md:p-6 rounded-xl md:rounded-2xl border border-gray-100 hover:border-brand-primary/20 transition-all cursor-pointer group">
                  ...
                  <h4 className="text-lg md:text-xl font-serif font-bold mb-2 group-hover:text-brand-primary transition-colors line-clamp-1 md:line-clamp-none">{post.title}</h4>
                  <div className="flex items-center gap-4 text-gray-400 text-sm">
                    <span className="flex items-center gap-1"><MessageSquare size={14} /> {post.commentsCount || 0}</span>
                    <span className="flex items-center gap-1"><Heart size={14} /> {post.likesCount || 0}</span>
                  </div>
              ...
              <div className="bg-white p-8 md:p-12 rounded-xl md:rounded-2xl border border-gray-100 text-center italic text-gray-400">暂无社区动态</div>
            )}
          </div>
        </div>

        <div className="bg-brand-primary rounded-2xl md:rounded-3xl p-6 md:p-10 text-gray-900 flex flex-col justify-between h-full min-h-[300px] md:min-h-[400px] shadow-lg md:shadow-xl">
          <div>
            <h2 className="text-2xl md:text-3xl font-serif font-bold mb-4 md:mb-6">加入我们</h2>
            <p className="text-gray-800/70 font-serif italic leading-relaxed mb-6 md:mb-8">"诗扶小筑是一个由粉丝自发维护的社区。无论你是资深乐迷，还是刚被圈粉的新人，这里都有你的位置。"</p>
          </div>
          <div className="space-y-3 md:space-y-4">
            <div className="flex items-center gap-3 p-3 md:p-4 bg-white/20 rounded-xl md:rounded-2xl border border-white/20">
              <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-white/40 flex items-center justify-center"><Music size={18} /></div>
              <div>
            ...
            <div className="flex items-center gap-3 p-3 md:p-4 bg-white/20 rounded-xl md:rounded-2xl border border-white/20">
              <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-white/40 flex items-center justify-center"><MessageSquare size={18} /></div>
              <div>
