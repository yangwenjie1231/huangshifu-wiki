...
import { Bell, CheckCheck, ChevronLeft, ChevronRight, Filter, MessageCircle, ThumbsUp, ShieldCheck } from 'lucide-react';
...
function isNotificationFilter(value: string | null): value is NotificationFilter {
  return value === 'all' || value === 'unread' || value === 'reply' || value === 'like' || value === 'review_result';
}
...
const Notifications = () => {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      if (filter === 'reply' || filter === 'like' || filter === 'review_result') {
        query.type = filter;
      }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <div className="mb-6">
        <Link to="/" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-brand-olive transition-colors">
          <ChevronLeft size={16} />
          返回首页
        </Link>
      </div>
      ...
      <section className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden">
        <header className="px-6 md:px-8 py-6 border-b border-gray-100">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">未读 {data.unreadCount} 条，共 {data.total} 条</p>
            </div>
            <button
            ...
            {FILTER_OPTIONS.map((option) => (
              <button
                ...
                className={clsx(
                  ...
                  filter === option.id ? 'bg-brand-olive text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'
                )}
              >
                {option.label}
              </button>
        ...
        <div className="min-h-[360px]">
          {loading ? (
            <div className="p-8 space-y-3">
              {[1, 2, 3, 4].map((item) => (
                <div key={item} className="h-20 bg-gray-50 rounded-2xl animate-pulse" />
              ))}
            </div>
            ...
            <ul>
              {data.notifications.map((notif) => {
                ...
                return (
                  <li key={notif.id} className={clsx('px-6 md:px-8 py-4 border-b border-gray-50 last:border-b-0', !notif.isRead && 'bg-sky-50/50')}>
                    <div className="flex items-start justify-between gap-4">
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-[11px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">{getNotificationTypeLabel(notif.type)}</span>
                          {!notif.isRead && <span className="text-[11px] text-sky-700 font-medium">未读</span>}
                        </div>
                        <p className={clsx('text-sm', !notif.isRead ? 'font-medium text-gray-900' : 'text-gray-700')}>{getNotificationText(notif)}</p>
                        <p className="text-xs text-gray-400 mt-1">{new Date(notif.createdAt).toLocaleString('zh-CN')}</p>
                      </button>
        ...
        <footer className="px-6 md:px-8 py-4 border-t border-gray-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => updateQuery(filter, Math.min(totalPages, page + 1))}
