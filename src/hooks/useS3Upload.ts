  ...
  for (let attempt = 1; attempt <= MAX_RETRIES + 1; attempt++) {
    try {
      ...
      if (attempt <= MAX_RETRIES) {
        console.log(
          `[S3 Upload] Upload attempt ${attempt} failed, retrying in ${RETRY_DELAY * attempt}ms...`
        )
  ...
  const upload = useCallback(
    async (file: File, options: UploadOptions = {}): Promise<{ key: string; md5: string }> => {
      ...
      try {
        ...
        const shouldCalculateMd5 =
          options.enableMd5Verification !== false || (await isServerMd5Required())
        ...
        console.log(
          '[S3 Upload] Presigned URL received, MD5 required:',
          presignResponse.md5Required
        )
