...
export function parseLRC(lrc: string): LRCData {
  ...
  for (const line of lines) {
    ...
    if (metaTag) {
      switch (metaTag.key) {
        case 'ti':
          metadata.title = metaTag.value
          break
        case 'ar':
          metadata.artist = metaTag.value
          break
        case 'al':
          metadata.album = metaTag.value
          break
        case 'lyricist':
          metadata.lyricist = metaTag.value
          break
        case 'composer':
          metadata.composer = metaTag.value
          break
        case 're':
          metadata.arranger = metaTag.value
          break
      }
      continue
    }
