...
function parseThresholdConfig(): ThresholdConfig {
  ...
  const warnThreshold = warnThresholdStr
    ? parseInt(warnThresholdStr, 10)
...
function printOverviewTable(stats: BuildStats): void {
  ...
  const rows = [
    ...
    ['平均文件大小', stats.fileCount > 0 ? formatBytes(Math.round(stats.totalSize / stats.fileCount)) : 'N/A'],
  ];
  ...
  // 打印表头
  console.log(
    `│ ${rows[0][0].padEnd(colWidths[0])} │ ${rows[0][1].padStart(colWidths[1])} │`
  );
  console.log(
    `├${'─'.repeat(colWidths[0] + 2)}┼${'─'.repeat(colWidths[1] + 2)}┤`
  );

  // 打印数据行
  for (let i = 1; i < rows.length; i++) {
    console.log(
      `│ ${rows[i][0].padEnd(colWidths[0])} │ ${rows[i][1].padStart(colWidths[1])} │`
    );
  }
...
function printLargestFilesTable(files: FileInfo[]): void {
  ...
  files.forEach((file, index) => {
    ...
    // 截断过长的文件路径
    const displayPath =
      file.relativePath.length > 55
        ? file.relativePath.slice(0, 52) + '...'
        : file.relativePath;
